 ...
 * 		$obj->waterStr = 'www.turingphp.com';			//水印文字
 *		$obj->fontSize = 16;						//文字字体大小
 *		$obj->fontColor = array(255,0,255);			//水印文字颜色（RGB）
 *		$obj->fontFile = = 'simhei.ttf';			//字体文件
...
class WaterMask{
	...
	public  $fontSize			= 40;				//文字字体大小
	public  $fontColor			= array(0,0,0);	//水印文字颜色（RGB）
	public  $fontFile			= 'simhei.ttf';	//字体文件

	public  $waterImg			= 'lf.gif';		//水印图片

	private $srcImg				= 'ayx.jpg';		//需要添加水印的图片
	public  $mbImg				= '' ;	//需要添加水印的图片
	private $im					= '';				//图片句柄
     ...
     * @param 缩略图宽度 $width
     * @param 缩略图高度 $height
     * @param 缩略图文件名 $thumbfile
     */
    function thumbimg($width,$height,$thumbfile){
        $this->imginfo();
        $imm=imagecreatetruecolor($width,$height);
        imagecopyresized($imm,$this->im,0,0,0,0,$width,$height,$this->srcImg_info[0],$this->srcImg_info[1]);
        switch ($this->srcImg_info[2]) {
        	case 1:
        		   imagegif($imm,$thumbfile.'.gif');
        		   break;
        	case 2:
        		   imagejpeg($imm,$thumbfile.'.jpg');
        		   break; 
        	case 3:
        		   imagepng($imm,$thumbfile.'.png');
        		   break;        		          		   
        }
